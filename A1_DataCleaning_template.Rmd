...
```{r}
# Extracting the numbers from the vector (VISIT in token), which also includes letters
...
```{r}
# Replacing all "." with nothing to delete them
...
demoselected <- select(demo, c(1:4, 7, 9, 14, 22, 24, 33))
LUselected <- select(LU, c(1:3, 8))
tokenselected <- select(token, c(1:4, 6, 7))
```
...
```{r}
# Merging by Subject and visit - (all = TRUE (el. all.x = T og all.y = T) --> matcher, hvad der matches kan, og resten er tilføjet med NA fyld ud i de ekstra "huller")
df <- merge(demoselected, LUselected, by = c("SUBJ", "VISIT"), all = TRUE)
df2 <- merge(df, tokenselected, by = c("SUBJ", "VISIT"), all = TRUE)
...
# Renaming the variables to indicate, they only include data from visit 1
colnames(visit1)[2] <- "ADOS1"
colnames(visit1)[3] <- "SOCIALISATION1"
colnames(visit1)[4] <- "MULLENRAW1"
colnames(visit1)[5] <- "EXPRESSIVELANGRAW1"

# Merging the new dataset with the old
df3 <- merge(visit1, df2, by = "SUBJ")
...
df3$GENDER <- ifelse(df3$GENDER == 1, "M", df3$GENDER) # "M" instead of 1 - indicating "Male"
df3$GENDER <- ifelse(df3$GENDER == 2, "F", df3$GENDER) # "F" instead of 2 - indicating "Female"

df3$DIAGNOSIS <- ifelse(df3$DIAGNOSIS == "A", "ASD", df3$DIAGNOSIS) # "ASD" instead of "A" - indicating "Autism Spectrum Disorder"
df3$DIAGNOSIS <- ifelse(df3$DIAGNOSIS == "B", "TD", df3$DIAGNOSIS) # "TD" instead of "B" - indicating "Typically Developing"
```
